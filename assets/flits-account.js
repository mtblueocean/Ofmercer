...
window.flitsApp.setupContainer = function () {
    if (flitsAppJquery(window).width() < 992) {
        ...
        flitsAppJquery(".flits-nav-bar").css('height', flitsAppJquery(".flits-pages").parent().height());
    }
};
            ...
            success: function (res) {
                if (res.status) {
                    ...
                    flitsAppJquery(".flits-hide-profile-edit").each(function (index, item) {
                        $(item).html($(item).next("input").val());
                    });
                    var gender = $form.find("select[name=gender]").val();
                    $form.find("select[name=gender]").closest(".flits-row").find('.flits-hide-profile-edit').html(gender);
            ...
            success: function (res) {
                if (res.status) {
                    ...
                    $li.find(".flits-remove-address-btn")
                            .attr("data-form-id", adrs.id);
                    if (adrs.default) {
                        $(".flits-addresses-ul .flits-address-default .flits-address .flits-row:first strong").html("&nbsp;");
                        $li.find(".flits-address .flits-row:first strong").html("Default");
                        $li.addClass("flits-address-default");
                        $(".flits-addresses-ul").prepend($li);
                    } else {
                        if (form_type == "edit") {

                        } else {
                            $(".flits-addresses-ul .flits-new-address-li").before($li);
                        }
                    }
                    if (form_type == "edit") {
...
//                console.log(res);
    ...
    flitsAppJquery(document).on('click', '.flits-order-re-order-btn', function () {
        var items = window.flits_orders[$(this).attr('data-target')];
