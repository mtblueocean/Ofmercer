...
//             flitsAppJquery('.flits-referrer-code').val(customer.referral_code);
          ...
          if (res.refer_by != '') {
            ...
            flitsAppJquery('.flits-refer-by-text').html(window.flitsApp.multilang_refer_friend.you_are_referd_by + " " + res.refer_by);
          }
          ...
          if (credit_log.length > 0) {
            flitsAppJquery.each(credit_log, function (index, item) {
              ...
              $li.find(".flits-referral-customer-name-col").html(item['referrer_customer'].name);
              $li.find(".flits-referral-customer-email-col").html(item['referrer_customer'].email).attr("title", item['referrer_customer'].email);
  ...
  this.copyReferrerCode = function () {
    flitsAppJquery("#flits-page-refer-friend").toast(window.flitsApp.multilang_refer_friend.referral_code_copied);
    ...
    referrer_message_text = flitsAppJquery.trim(referrer_message_text.replace("##referral_code##", referrer_code));
    ...
    if(refer_friend_code != ''){
      ...
      flitsAppJquery.ajax({
        ...
        success: function (res) {
          ...
          if (res.status) {
            flitsAppJquery("#flits-page-refer-friend").toast(window.flitsApp.multilang_refer_friend.successfully_refer_friend);
            var refer_by = res.data.inviter.name;
            if(refer_by != -1){
              flitsAppJquery('.flits-enter-referral-code-label').addClass('flits-hidden');
              flitsAppJquery('.flits-referee-inner-div').addClass('flits-hidden');
              flitsAppJquery('.flits-refer-by').removeClass('flits-hidden');
              flitsAppJquery('.flits-refer-by-text').html(window.flitsApp.multilang_refer_friend.you_are_referd_by + " " +  refer_by);
            }
          } else {
      ...
      flitsAppJquery(".flits-refer-loader").addClass('flits-hidden');
      flitsAppJquery(".flits-refer-friend-error-div").removeClass('flits-hidden');
      flitsAppJquery("#flits-page-refer-friend").toast(window.flitsApp.multilang_refer_friend.referral_code_is_required);
    }
